...
#Global variables that store the encryption keys in both hexadecimal and binary formats
...
# this is 0xEC 
ENCRYPT = 236 
#Global vars to store boxes & pub key (rest of encryption tools)
global commBox
global myPrivKey
myPrivKey = -1
global hostPubKey
...
class socket:
    ...
    def connect(self, *args):
        global ENCRYPT, mainSocket, currentSeqNo, commBox, publicKeys, defPubKey, hostPubKey, myPrivKey, defPrivKey
        if (len(args) >= 1): 
            address = args[0] 
            ...
            if (args[1] == ENCRYPT):
                ...
                for private_address in privateKeys:
                    if(private_address == (address[0],str(receiver))):
                        myPrivKey = privateKeys[private_address]
                        break
                if(myPrivKey == -1):
                    myPrivKey = defPrivKey
                if(myPrivKey == -1):
                ...
                for public_address in publicKeys:
                    ...
                    if(public_address == (address[0], str(transmitter)) ):
                        hostPubKey = publicKeys[public_address]
                        break
                if(hostPubKey == -1):
                    hostPubKey = defPubKey
                if(hostPubKey == -1):
                    print("\n no pub key found")
                    return
                print('\tThis is my secret key | their private key:\t %s | %s' % (myPrivKey, hostPubKey))
                commBox = Box(myPrivKey,hostPubKey)
            else:
                return
    ...
    def accept(self):
        ...
        if(len(args) >=1)
            if (args[0] == ENCRYPT):
                ...
                tempOtherHost = (addr[0],str(addr[1]))
                for private_address in privateKeys:
                    if(private_address == ('127.0.0.1',str(receiver))):
                        mPrivKey = privateKeys[private_address]
                        break
                if(myPrivKey == -1):
                    myPrivKey = defPrivKey
                if(myPrivKey == -1):
